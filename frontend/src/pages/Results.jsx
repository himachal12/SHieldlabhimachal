...
export default function Results() {
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="text-center">
        <Shield className="text-accent mx-auto animate-pulse mb-3" size={40} />
  ...
  // Risk level label based on critical/high count
  const riskLevel = results.critical_count > 0 ? 'CRITICAL'
    : results.high_count > 0 ? 'HIGH'
    : results.medium_count > 0 ? 'MEDIUM' : 'LOW'
  ...
  return (
    <div className="min-h-screen px-4 py-8 max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <span className="font-mono text-xs text-slate-500 ml-1">{scanId}</span>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
        ...
        {[
          { label: 'Total', value: results.total_findings, color: 'text-white' },
          { label: 'Critical', value: results.critical_count, color: 'text-red-400' },
          { label: 'High', value: results.high_count, color: 'text-orange-400' },
          { label: 'Medium', value: results.medium_count, color: 'text-yellow-400' },
        ].map(stat => (
          <div key={stat.label} className="bg-surface border border-slate-700 rounded-xl p-4">
      ...
      <div className="grid md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="md:col-span-2 bg-surface border border-slate-700 rounded-xl p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-white text-sm">Filter by Severity</h3>
          </div>
