...
def test_dedupe_findings_for_chain_analysis_keeps_highest_confidence_duplicate():
    findings = [
        {
            "finding_id": "bandit_secret",
            "vuln_type": "Hardcoded Secrets",
            "source": "bandit",
            "file_path": "app.py",
            "line_number": 15,
    ...
    deduped = analyzer._dedupe_findings_for_chain_analysis(findings)

    assert len(deduped) == 1
    assert deduped[0]["finding_id"] == "custom_secret"


def test_humanize_fix_order_removes_internal_finding_ids():
    assert analyzer._humanize_fix_order([
        "Fix finding_e7e9ff02 by restricting access to the Git repository configuration file",
        "Rotate finding_5c69a8ef immediately",
    ]) == [
        "Restricting access to the Git repository configuration file",
        "Rotate the linked finding immediately",
    ]
