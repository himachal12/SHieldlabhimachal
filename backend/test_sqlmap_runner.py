...
def test_successful_launch(sqlmap_config, monkeypatch):
    FakePopen.calls = []
    monkeypatch.setattr(sqlmap_runner.subprocess, "Popen", FakePopen)
    ...
    assert command[-2:] == ["--version", "--batch"]
...
def test_timeout(sqlmap_config, monkeypatch):
    monkeypatch.setattr(sqlmap_runner.subprocess, "Popen", TimeoutPopen)
