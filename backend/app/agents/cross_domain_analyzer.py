...
{{
  ...
  "recommended_fix_order": ["human-readable first fix without raw finding IDs", "human-readable second fix without raw finding IDs"]
}}"""
...
def _dedupe_findings_for_chain_analysis(findings: list[dict]) -> list[dict]:
    """Remove scanner-duplicate findings so the UI does not show repeated chains.

    Bandit and custom detectors can report the same hardcoded secret with slightly
    different line numbers. For chain analysis we keep the highest-confidence
    instance for the same vulnerability type/evidence/location shape. The raw
    findings still remain visible in the finding explorer; this only reduces
    threat-path noise.
    """
    best_by_key: dict[tuple, dict] = {}
    for finding in findings:
        evidence_text = (finding.get("vulnerable_code") or finding.get("description") or "").strip()
        key = (
            finding.get("vuln_type"),
            finding.get("repository_relative_path") or finding.get("file_path") or finding.get("url") or finding.get("port"),
            evidence_text,
        )
...
def _humanize_fix_order(items: list) -> list[str]:
    """Strip raw internal finding IDs from LLM fix-order bullets."""
    humanized = []
    for item in items or []:
        text = str(item).strip()
        text = re.sub(r"^fix\s+finding_[a-f0-9]+\s+by\s+", "", text, flags=re.IGNORECASE)
        text = re.sub(r"\bfinding_[a-f0-9]+\b", "the linked finding", text, flags=re.IGNORECASE)
        if text:
            humanized.append(text[:1].upper() + text[1:])
    return humanized
...
def analyze_attack_chains(findings: list[dict]) -> list[dict]:
    ...
    code_findings = _dedupe_findings_for_chain_analysis([f for f in findings if _scanner_family(f) == "code"])
    web_findings = _dedupe_findings_for_chain_analysis([f for f in findings if _scanner_family(f) == "web"])
