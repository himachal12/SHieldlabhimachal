...
def _subprocess_startup_flags() -> int:
    """Return Windows-only flags that keep SQLMap detached from console input."""
    flags = 0
...
def _sqlmap_environment() -> dict[str, str]:
    """Create a subprocess environment optimized for non-interactive output."""
    env = os.environ.copy()
...
def _read_stream(stream: TextIO | None, name: str, output: list[str]) -> None:
    """Continuously drain a SQLMap stream so the process cannot block on full pipes."""
    if stream is None:
        return
    try:
        for line in iter(stream.readline, ""):
            ...
            if clean_line:
                logger.info(f"SQLMap {name}: {clean_line}")
    finally:
        stream.close()
...
def execute_sqlmap(command: Sequence[str], timeout: int = DEFAULT_SQLMAP_TIMEOUT) -> SQLMapResult:
    """
    Launch SQLMap non-interactively, stream output in background threads,
    enforce a hard timeout, and return structured diagnostics.
    """
    ...
    try:
        process = subprocess.Popen(
            ...
            env=_sqlmap_environment(),
            creationflags=_subprocess_startup_flags(),
        )

        stdout_thread = threading.Thread(
            target=_read_stream, args=(process.stdout, "stdout", stdout_lines), daemon=True
        )
        stderr_thread = threading.Thread(
            target=_read_stream, args=(process.stderr, "stderr", stderr_lines), daemon=True
        )
        stdout_thread.start()
        stderr_thread.start()
        ...
        logger.error(f"SQLMap timed out after {timeout}s; terminating process.")
    ...
    return SQLMapResult(list(command), returncode, stdout, stderr)
...
def is_sqlmap_available() -> bool:
    ...
    try:
        ...
        result = execute_sqlmap(
            [config.python_executable, config.sqlmap_path, "--version", "--batch"],
...
def get_sqlmap_unavailable_reason() -> str | None:
    """Return a UI/log friendly reason when SQLMap is misconfigured."""
    try:
        get_sqlmap_config()
        return None
    except SQLMapConfigurationError as exc:
